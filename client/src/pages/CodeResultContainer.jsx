...
class CodeResultContainer extends React.Component {
  ...
  async loadResult() {
    ...
    if (res.data.success) {
      ...
      if (this.state.data.Name.length > 0) {
        ...
        str +=
          "위 변수 중 활용된 변수는 " +
          this.state.data.UsedName.length +
          "개, 활용되지 않은 변수는 " +
          (this.state.data.Name.length - this.state.data.UsedName.length) +
          "개 입니다.\n";

        str +=
          "활용된 변수 중 다른 변수에 활용된 변수는 " +
          this.state.data.NameUsedAssign.length +
          "개, 연산에 활용된 변수는 " +
          this.state.data.NameUsedOp.length +
          "개 입니다.\n";
      }
      if (
        this.state.data.BinOp +
          this.state.data.AugAssign +
          this.state.data.Compare +
          this.state.data.Logical >
        0
      ) {
        str +=
          "연산자 활용 횟수는 총 " +
      ...
      if (this.state.data.For + this.state.data.While > 0) {
        str +=
          "반복문을 활용한 횟수는 총 " +
          (this.state.data.For + this.state.data.While) +
          "회 입니다.\n";
      ...
      if (this.state.data.If + this.state.data.ElseIf + this.state.data.Elif > 0) {
        str +=
          ...
          "회,  " +
          "if else 문을 총 " +
          this.state.data.ElseIf +
          "회,   " +
          "if elif else 문을 총 " +
          this.state.data.Elif +
          "회 사용하였습니다.\n";
      ...
      if (this.state.data.Function + this.state.data.UsedInnerFunc.length > 0) {
        str +=
          ...
          (this.state.data.Function + this.state.data.UsedInnerFunc.length) +
          "개, 활용되지 않은 함수는 " +
          this.state.data.UnusedFunc +
          "개 입니다.";
      }
  ...
  async saveResult() {
    const res = await postResult(
      this.state.data.Name.length,
      this.state.data.BinOp +
        this.state.data.AugAssign +
        this.state.data.Compare +
        this.state.data.Logical,
      0,
      this.state.data.If + this.state.data.ElseIf + this.state.data.Elif,
      this.state.data.For + this.state.data.While,
      this.state.data.Function
    );
                ...
                <Jumbotron>
                  ...
                  */}
                  <Row>
                    ...
                    {this.state.data != null &&
                      this.state.data.If + this.state.data.ElseIf + this.state.data.Elif !== 0 && (
                        <Col>
                          <FormCheck checked={true} />
                        </Col>
                      )}
                  ...
                  <Row>
                    ...
                    {this.state.data != null &&
                      (this.state.data.For !== 0 || this.state.data.While !== 0) && (
                        <Col>
                          <FormCheck checked={true} />
                        </Col>
                      )}
                    {this.state.data != null &&
                      this.state.data.For === 0 &&
                      this.state.data.While === 0 && (
                        <Col>
                          <FormCheck checked={false} />
                        </Col>
                      )}
