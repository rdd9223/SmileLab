...
const CommunityContainer = () => {
  ...
  }, [currentClass, currentPage, loadBoard, type]);
    ...
    if (type === 0) {
      return (
        <div style={{ borderRadius: 10, border: "1px solid #000", padding: 10 }}>
          문제해결에 대한 아이디어를 동료들에게 얻는 공간입니다. 내 아이디어에 동료의 아이디어가
          더해졌을 때, 더 정교하고 타당한 문제해결 방안을 찾을 수 있게 될 것입니다
        </div>
      );
    } else if (type === 1) {
      return (
        <div style={{ borderRadius: 10, border: "1px solid #000", padding: 10 }}>
          함께 프로젝트를 수행할 동료를 찾는 공간입니다. 동료와의 공동 프로젝트를 통해 조금 더
          복잡한 코드를 학습할 수 있고, 나의 부족한 부분을 동료가 채워줄 수도 있습니다. 나와 마음이
          맞는 동료를 찾아보세요.
        </div>
      );
    } else if (type === 2) {
      return (
        <div style={{ borderRadius: 10, border: "1px solid #000", padding: 10 }}>
          코드 작성 시 궁금증을 해결하기 위한 공간입니다. 이해하기 어려운 에러나 문제 개선에 대한
          궁금점 등을 질문하거나, 반대로 동료들에게 내 지식을 활용하여 조언을 할 수도 있습니다.
          질문을 하는것, 질문에 내 지식을 나누어 주는 것 또한 여러분의 학습능력과 사고력 향상을 위해
          매우 중요한 과정입니다.
        </div>
      );
    } else {
      return (
        <div style={{ borderRadius: 10, border: "1px solid #000", padding: 10 }}>
          내 코드를 공유하거나 동료의 코드를 리뷰하는 공간입니다. 코드에 대한 평가 뿐만 아니라
          동료의 코드를 재사용하여 아이디어를 더해 더 훌륭한 코드를 만들어 볼 수도 있습니다. 내 코드
          뿐만 아니라 다른 사람의 코드를 이해하고 개선시키는 것 또한 프로그래밍에 필요한 사고력을
          기르는데 매우 중요한 과정입니다.
        </div>
      );
    }
    ...
    return (
      <Wrapper>
        ...
        {data.map((item, idx) => {
          return (
            <div>
              <CommunityPostContainer item={item} currentUser={userIdx} />
            </div>
          );
        })}
        <Wrapper>
